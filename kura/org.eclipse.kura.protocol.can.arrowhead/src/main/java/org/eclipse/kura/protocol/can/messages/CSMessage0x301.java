 ...
 *
 */
public class CSMessage0x301 {
    private static final Logger s_logger = LoggerFactory.getLogger(ArrowheadCanSocketImpl.class);

    private CSMessage0x301() {
    }

    public static void parseCanMessage(CanMessage cm, PrivateCSDataSnapshot privateCSReceivedData) {
        byte[] b = cm.getData();
        if (b != null && b.length == 5) {
            StringBuilder sb = new StringBuilder("received 0x301: ");
            ...
            privateCSReceivedData.setRechargeAvail(rechargeAvailable);
            privateCSReceivedData.setRechargeInProgress(rechargeInProgress);
            privateCSReceivedData.setPVSystemActive(pvSystemActive);
            privateCSReceivedData.setAuxChargerActive(auxChargerActive);
            privateCSReceivedData.setStorageBatterySts(storageBatteryConcactorSts);
            privateCSReceivedData.setConverterContactorSts(converterConcactorSts);
            privateCSReceivedData.setIGBTTemp(igbtTemperature);
            privateCSReceivedData.setStorageBatteryTemp(storageBatteryTemperature);
            privateCSReceivedData.setStorageBatterySOC(storageBatterySOC);
        }
    }
