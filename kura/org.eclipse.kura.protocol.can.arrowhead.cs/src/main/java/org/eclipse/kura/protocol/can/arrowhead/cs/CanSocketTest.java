...
public class CanSocketTest implements ConfigurableComponent {
	...
	private static final String RECHARGE_DURATION = "arrowhead.recharge.duration";
	...
	private GWControlStatus controlStatus;
	private RechargeSimulationThread rechargeSimulationThread;

	enum RechargeState {IDLE, RECHARGE_IN_PROGRESS, WAITING_ACK};
	private RechargeState rechargeState = RechargeState.IDLE;
	...
	protected void activate(ComponentContext componentContext, Map<String, Object> properties) {
		...
		controlStatus = new GWControlStatus();
		rechargeSimulationThread = new RechargeSimulationThread();
		rechargeSimulationThread.start();
		...
		if (m_properties != null) {
			...
			rechargeDuration = (Long) m_properties.get(RECHARGE_DURATION);
			
			s_logger.info("recharge duration " + rechargeDuration);
		}
	...
	protected void deactivate(ComponentContext componentContext) {
		...
		rechargeSimulationThread.terminate();
	}
	...
	private void setRechargeState(RechargeState newState) {
		s_logger.info("state change: " + rechargeState + " -> " + newState);
		rechargeState = newState;
	}
	
	private void handle0x200Message() {
		boolean startRecharge = controlStatus.isStartRecharge();
		
		switch (rechargeState) {
		case IDLE:
			if (startRecharge) {
			rechargeSimulationThread.startRecharge();
			}
			break;
		case RECHARGE_IN_PROGRESS:
			if (!startRecharge) {
			rechargeSimulationThread.stopRecharge();	
			}
			break;
		case WAITING_ACK:
			if (!startRecharge) {
	...
	private void sendMessage0x100(String ifName) throws KuraException, IOException {
	    ...
	    // s_logger.info("Message sent with id: 0x" + Integer.toHexString(id));
	}
	...
	private void sendMessage0x101(String ifName) throws KuraException, IOException {
		...
		// Status
		// [0,1]
		int storageBatteryChargerStatus = (message0x101Info.isStorageBatteryChargerStatus() << 7); // Storage
																									// Battery
																									// Charger
																									// Status
		...
		bMessage[0] = (byte) (faultFlag + rechargeAvailable + rechargeInProgress + pvSystemActive + auxChargerActive
				+ storageBatteryContractorStatus + converterContractorStatus + storageBatteryChargerStatus);
	...
	private void sendMessage0x301(String ifName) throws KuraException, IOException {
		...
		bMessage[0] = (byte) (faultFlag + rechargeAvailable + rechargeInProgress + pvSystemActive + auxChargerActive
				+ storageBatteryContractorStatus + converterContractorStatus);
	...
	private class RechargeSimulationThread extends Thread {
		...
		public synchronized void startRecharge() {
			if (message0x101Info.isRechargeInProgress() != 0) {
				return;
			}
